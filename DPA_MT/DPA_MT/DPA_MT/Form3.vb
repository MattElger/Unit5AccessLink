...
Public Class Form3
    ...
    Private Sub GetAllSuppliers()
        ...
        Do While DR.Read
            'add to ComboBox
            cmbSuppAll.Items.Add(DR.Item("SupplierID").ToString & " " & DR.Item("Name").ToString)
        Loop
        CN.Close()
        'select top row of ComboBox
        cmbSuppAll.SelectedIndex = 0
    End Sub
    ...
    Private Sub GetItemSuppliers(ByVal strID As String)
        ...
        Dim strSQL As String = "SELECT tblSupplier.SupplierID, Name FROM tblSupplier, tblSupplierItem WHERE tblSupplierItem.SupplierID = tblSupplierItem.SupplierID AND StockID = '" & strID & "'"
        'get records from database into DataReader
        CN.Open()
        Dim CM As New OleDbCommand(strSQL, CN)
        Dim DR As OleDbDataReader = CM.ExecuteReader
        'initialise empty
        cmbSuppItem.Items.Clear()
        cmbSuppItem.Text = ""
        'for each record
        Do While DR.Read
            'add to ComboBox
            cmbSuppItem.Items.Add(DR.Item("SupplierID").ToString & " " & DR.Item("Name").ToString)
        Loop
        CN.Close()
        'if ComboBox not empty
        If cmbSuppItem.Items.Count > 0 Then
            'select top row
            cmbSuppItem.SelectedIndex = 0
        End If
    End Sub
    ...
    Private Sub btnInsert_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnInsert.Click
        ...
        Dim strSupplierID As String = cmbSuppItem.Text.Substring(0, cmbSuppItem.Text.IndexOf(Chr(32)))
    ...
    Private Sub btnDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        ...
        Dim strStockID As String = txtStockID.Text
        If strStockID = "" Then Exit Sub
        Dim strSupplierID As String = cmbSuppItem.Text.Substring(0, 2)
        If strSupplierID = "" Then Exit Sub
